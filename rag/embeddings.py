...
class Embedder:
    ...
    def embed(self, texts, normalize=False):
        ...
        When `normalize` is True the vectors are L2-normalized (unit length) so
        that an inner-product search (FAISS IndexFlatIP) yields cosine similarity.
        """
        ...
        # Encode the texts into vectors. normalize_embeddings handles cosine
        # normalization for us; FAISS requires float32, so cast to it.
